...
> Status (#0025): a camada de emissão automática **está implementada** — `lib/fiscal/` (abstração neutra + `AsaasFiscalGateway`) + `POST /api/cron/nfe` (`pg_cron` de hora em hora) emitem a NFS-e de cada fatura paga ainda sem nota (com teto de tentativas e alerta), colunas `nfe_*` em `invoices` e retenção fiscal em `invoice_archive`. Fica **desligada por padrão** (`FISCAL_PROVIDER=none`, no-op); ligar exige a parte operacional — módulo de NF-e habilitado no Asaas + inscrição municipal + código de serviço + contador.
