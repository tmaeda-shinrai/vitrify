...
- **Moderação (#0023):** `vitrines` e `profiles` ganharam `blocked_at`/`blocked_reason`; a policy `vitrines_public_read` passou a exigir `blocked_at IS NULL` e o trigger `protect_block_fields` (SECURITY INVOKER) só deixa `service_role` mudar o bloqueio. Nova tabela **`reports`** (denúncias: `vitrine_id`, `reporter_email`, `reason`, `description`, `status`, `admin_notes`, `resolved_at`) com RLS habilitada **sem policy** (só service role). RPC **`admin_metrics()`** (SECURITY DEFINER, revogada de anon/authenticated) agrega DAU/MAU (atividade em `audit_logs`) + funil para o `/admin`.
- **Embaixadoras (#0025):** `profiles` ganhou `is_ambassador`/`ambassador_since`; o trigger `protect_profile_ambassador` (SECURITY INVOKER, espelha `protect_block_fields`) só deixa `service_role` alterar essas colunas — a dona não se autoconcede o selo editando o próprio perfil. A concessão (admin, `setAmbassadorAction`) põe a assinatura em `plus`/`active` sem fim de período (Plus vitalício, reusa o gating de plano) e exibe o selo "Embaixadora Pioneira" na vitrine pública; encerrar volta para `free` (produtos preservados).
- **NF-e / NFS-e (#0025):** `invoices` ganhou `nfe_status` (NULL=pendente | `issued` | `failed`), `nfe_id`, `nfe_url`, `nfe_issued_at`, `nfe_attempts`, `nfe_error` + índice parcial `idx_invoices_nfe_pending` (faturas pagas ainda sem nota). `POST /api/cron/nfe` (pg_cron hourly) emite a NFS-e pelo **módulo de NF-e do Asaas** (`lib/fiscal/`, no-op sem `FISCAL_PROVIDER`) — **nenhum dado fiscal pessoal é persistido**: o Asaas já tem o cliente da cobrança (forward-only). `invoice_archive` ganhou `nfe_id`/`nfe_url` e `hard_delete_account` passou a arquivá-los (retenção fiscal de 5 anos).
