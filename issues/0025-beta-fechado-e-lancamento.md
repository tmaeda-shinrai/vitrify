...
- [x] NF-e automática (módulo do Asaas) — `lib/fiscal/` + `POST /api/cron/nfe` (`pg_cron`), colunas `nfe_*` em `invoices` + retenção fiscal 5 anos (`invoice_archive`); desligada por padrão (`FISCAL_PROVIDER`), ligar é operacional (módulo/inscrição/contador) — movida da #0024
