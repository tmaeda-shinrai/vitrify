...
- **M3 — Pedidos e estatísticas (em andamento):** #0015 (core) intenções de pedido — `POST /api/intent` (anon, sem cookies; rate-limit 10/min por IP + dedup curto por `ip_hash`+`product_id` via Upstash; **SHA-256 do IP** (`hashIp`, nunca o IP cru); user-agent resumido a 3 categorias; `source` derivado do referrer — `lib/intent-source.ts`, `lib/validators/intent.ts`) insere em `order_intents`; o trigger `bump_product_intents` soma `products.intents_count`. Tela **Pedidos** (`app/(dashboard)/pedidos`) com feed agrupado por dia (`lib/intents.ts` + `IntentsFeed`), produto/horário/dispositivo e origem (Pro+); `recordOrderIntent` (#0013) agora envia `referrer`. **Falta:** contagem de **views** da vitrine (PR2).
...
**Not built yet:** #0016 onward (estatísticas, PWA, Asaas billing, etc.) e a contagem de **views** da vitrine (PR2 do #0015). `lib/asaas/` is still a stub.
